...
def process_data():
    ...
    df = pd.read_csv('../rawData/Tabla Python Dimar - Bain - Satisfaccion.csv')
    ...
    for tool in df['Herramienta Gerencial'].unique():
        ...
        output_df = pd.DataFrame({
            'Year': tool_data['Year'],
            tool: tool_data['Satisfacción']
        })
        
        # Generate filename
        clean_name = clean_filename(tool)
        random_id = generate_random_id()
        filename = f'BS_{clean_name}_{random_id}.csv'
    ...
    index_df.to_csv('../NewDBase/BSIndex.csv', index=False)
