...
from notas import gt_note, gbn_note, bu_note, cr_note, bs_note
...
sidebar = html.Div(
    [
        # Content wrapper div to allow scrolling while keeping footer fixed
        html.Div([
            # Logo
            html.Img(
            ...
            html.Div([
                ...
                html.Div(
                    "NOTAS",
                    style={
                        'backgroundColor': '#4a8a89',  # Bootstrap's secondary color (dark grey)
                        'color': 'white',
                        'padding': '5px 10px',
                        'fontSize': '12px',
                        'fontWeight': 'bold',
                ...
                html.Div([
                    ...
                    dbc.Collapse(
                        dbc.Card(dbc.CardBody(gt_note, style={'fontSize': '10px'})),
                        id="collapse-gt-note",
                        is_open=False,
                    ),
                ], className="mb-1"),
                # Google Books Ngram Note
                html.Div([
                    dbc.Button(
                        "Google Books Ngram",
                        id="toggle-gbn-note",
                        ...
                        outline=True,
                        size="sm",
                        className="mb-1 w-100",  # Changed from "mb-2 w-100" to "mb-1 w-100"
                        style={'fontSize': '12px'}
                    ),
                    dbc.Collapse(
                        dbc.Card(dbc.CardBody(gbn_note, style={'fontSize': '10px'})),
                        id="collapse-gbn-note",
                        is_open=False,
                    ),
                ], className="mb-1"),  # Changed from "mb-2" to "mb-1"
                # Bain Usability Note
                html.Div([
                    dbc.Button(
                        "Bain - Usabilidad",
                        ...
                        outline=True,
                        size="sm",
                        className="mb-1 w-100",  # Changed from "me-2 mb-2" to "mb-1 w-100"
                        style={'fontSize': '12px'}
                    ),
                    dbc.Collapse(
                        dbc.Card(dbc.CardBody(bu_note, style={'fontSize': '10px'})),
                        id="collapse-bu-note",
                        is_open=False,
                    ),
                ], className="mb-1"),  # Changed from "mb-2" to "mb-1"
                ...
                html.Div([
                    ...
                    dbc.Collapse(
                        dbc.Card(dbc.CardBody(cr_note, style={'fontSize': '10px'})),
                        id="collapse-cr-note",
                        is_open=False,
                    ),
                ], className="mb-1"),  # Changed from "mb-2" to "mb-1"
                # Bain Satisfaction Note
                html.Div([
                    dbc.Button(
                        "Bain - Satisfacción",
                        id="toggle-bs-note",
                    ...
                    dbc.Collapse(
                        dbc.Card(dbc.CardBody(bs_note, style={'fontSize': '10px'})),
                        id="collapse-bs-note",
                        is_open=False,
                    ),
                ], className="mb-1"),  # Changed from "mb-2" to "mb-1"
            ], id='notes-section', style={
                'marginBottom': '60px',
                'display': 'none'
            }),
        ], style={
            'overflowY': 'auto',
            'overflowX': 'hidden',
            'height': 'calc(100vh - 80px)',  # Reserve space for footer
            'paddingRight': '10px'
        }),

        # Footer div - now outside the scrollable area
        html.Div([
            html.Hr(),
            html.P([
                "Desarrollado por: ",
...
# Add callbacks for note toggles
for db in ['gt', 'gbn', 'bu', 'cr', 'bs']:
    @app.callback(
        Output(f"collapse-{db}-note", "is_open"),
        [Input(f"toggle-{db}-note", "n_clicks")],
        [State(f"collapse-{db}-note", "is_open")],
    )
    def toggle_note(n_clicks, is_open):
        if n_clicks:
            return not is_open
        return is_open
...
# Add new callback to control Notes section visibility
@app.callback(
    Output('notes-section', 'style'),
    [Input('keyword-dropdown', 'value')] +
    [Input(f"toggle-source-{id}", "outline") for id in dbase_options.keys()]
)
def toggle_notes_section(selected_keyword, *button_states):
    # Convert button states to selected sources
    selected_sources = [id for id, outline in zip(dbase_options.keys(), button_states) if not outline]
